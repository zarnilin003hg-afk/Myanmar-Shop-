              ...
              <div key={index} className="flex items-center p-3 rounded-lg bg-gray-50 border">
                <div className="flex-1">
                  <div className="font-semibold text-gray-800">
                    {entry.old_price.toLocaleString()} ကျပ် <span className="text-gray-400 mx-1">→</span> {entry.new_price.toLocaleString()} ကျပ်
                  </div>
                  <div className="text-sm text-gray-500">
                    {new Date(entry.date).toLocaleDateString('en-CA')}
                  </div>
                </div>
                <div className={`font-bold text-lg ${isIncrease ? 'text-red-500' : 'text-green-600'}`}>
