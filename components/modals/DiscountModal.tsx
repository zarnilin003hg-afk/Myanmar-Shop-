...
export const DiscountModal: React.FC<DiscountModalProps> = ({ currentDiscount, cart, selectedCustomer, onClose, onApply, addToast }) => {
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <label className="block text-sm font-semibold mb-2 text-gray-700">လျှော့စျေးပမာဏ (ကျပ်)</label>
        <input 
          type="number" 
          value={discount}
          ...
          min="0" 
          max={subtotal}
          className="w-full px-4 py-3 rounded-lg border-2 text-lg border-gray-300"
        />
      </div>
      <div className="flex gap-3">
        ...
        <button onClick={onClose} className="flex-1 px-6 py-3 rounded-lg font-semibold bg-gray-200 text-gray-700 hover:bg-gray-300">
          ပယ်ဖျက်မည်
        </button>
      </div>
    </div>
